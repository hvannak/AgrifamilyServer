...
router.post('/register', async (req,res) => {
    ...
    if(emailExist) return res.status(400).send('RExistEmailMessage');
    //Hash password
    ...
    try{
        ...
        res.send({_id:user._id,message:'RSuccessMessage'});
    }catch(err){
...
router.post('/loginclient', async (req,res) => {
    try {
        ...
        const user = await User.findOne({email: req.body.email});
        // if(!user) return res.status(400).send('Email or password is wrong');
        if(!user) return res.status(400).send('CWrongEmailPassMessage');
        const validPass = await bcrypt.compare(req.body.password,user.password);
        // if(!validPass) return res.status(400).send('Invalid password');
        if(!validPass) return res.status(400).send('CInvalidPassMessage');
        //Create and assign a token
        const token = jwt.sign({_id: user._id},process.env.TOKEN_SECRET);
        res.header('auth-token',token).send(token);
        // res.headers("authorization", `Bearer ${token}`).send();
