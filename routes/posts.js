...
router.get('/hotPost', async (req,res) => {
    try{
        const result = await Post.find().limit(12).sort({
            date: 'desc'
        });
        res.json(result);
    }catch(err){
...
router.get('/hotPostCategory/:langId', async (req,res) => {
    try{
        var result = [];
        const categoryObj = await Category.find({lang: req.params.langId});
        for (const item of categoryObj) {
            const postCollection = await Post.find({category: item._id}).limit(3).sort({
                date: 'desc'
            });
...
router.post('/searchByCat',async (req,res) => {
    try{
        ...
        var docObj;
        var filter;
        if(req.body.searchObj == null){
            filter = {};
        } else{
            filter = (req.body.categoryId != '-1') ? {
...
router.post('/searchdetails',async (req,res) => {
    try{
        ...
        var currentPage = opt.page;
        var docObj;
        var filterObj = req.body.searchObj == null ? req.body.searchpostmodel : req.body.searchObj;
        let dynamicQuery = {};

        if(req.body.categoryId != '-1'){
            dynamicQuery["category"] = req.body.categoryId
        }
        if(filterObj.title != ""){
